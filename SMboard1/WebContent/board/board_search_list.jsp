				...
				<tbody>
					...
					<tr>
						...
						<c:if test="${!empty search.answer_lev}">
							<c:forEach var="j" begin="0" end="${search.answer_lev*2}" step="1">
								&nbsp;
							</c:forEach>
						</c:if>
					...
					<tbody>
						...
						<tr>
							...
							<c:choose>
								...
								<c:otherwise>
									<a href="./BoardSearch.do?page=<c:out value="${page-1}"/>">[이전]</a>&nbsp;
								</c:otherwise>
							</c:choose>
							...
							<c:choose>
								...
								<c:otherwise>
									<a href="./BoardSearch.do?page=<c:out value="${start}" />">[<c:out value="${start}" />]</a>&nbsp;
								</c:otherwise>
							</c:choose>
							</c:forEach>
							<c:choose>
								...
								<c:otherwise>
									<a href="./BoardSearch.do?page=<c:out value="${page+1}" />">[다음]</a>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
